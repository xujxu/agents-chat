...
- [x] Add a test covering UTF8, UTF8 BOM and UTF16LE BOM:
...
- [x] Add a launcher/path serialization assertion:
...
- [x] Commit/push tests and the Linux runner selector. Expected failure is missing
...
- [x] Export these functions from `configuration-files.mjs`:
...
- [x] Read only ordinary single-link files; reject symlink/hardlink targets.
...
- [x] Extend the managed-key regex with `LAUNCHER_PATH`. Include
  ...
  before existing quoting, and reject embedded double-quote/control/expansion
  characters. Apostrophes are valid inside double-quoted paths.
  Preserve Linux backslash semantics and unrelated configuration text.

## Task 3: Windows private writes, before any secret bytes

- [x] Add the PowerShell helper taking only a destination-directory path through
...
- [x] Write the temp file within that private directory, flush/close, then
...
- [x] Add `windows-files` job on `windows-2022`, Node24.20.0, running:
...
```
  ...
  access, refuse linked/existing directory cases and directory destinations,
  and require old config plus no leaked temporary stages. Directory destinations
  are rejected at preflight; stale-byte refusal exercises cleanup after staging.
...
- [x] Run existing Linux configuration/release, PTY and isolated deploy tests;
...
- [x] Record exact commit/run evidence in this plan and the ledger. Keep the
  Windows model-selection gate closed; do not describe private-file success as
  installation/upgrade or Win11 acceptance.

## Execution evidence

Test-first `59b299f`, run `35983021333`: intended missing-module failure on both
platforms. Implementation `23dfb21`, snapshot/receipt guard `3bd6b28`.
Windows helper timed out at PowerShell `Test-Path`; controlled diagnostics
`35983725074` isolated the operation. `15f4ce0` replaces module-dependent cmdlets
with .NET APIs. No ACL assertions were removed. Diagnostic scaffolding was
removed after successful run `35984024095`.

Final code `fd80b9a`, Actions `35984193290`: five Windows file/CLI contracts pass;
Linux twelve passes and one intentional Windows-only skip, PTY/deploy regressions
and both real package integrity jobs pass. Tests include exact UTF16LE recovery,
final file ACL identities/rights, junction refusal, invalid recovery encoding,
apostrophe paths and stale-config protection. No model build or local validation.

The existing PowerShell startup scripts' other configuration writes, service
identity/environment overrides, authenticated installed-model activation and
setup/deploy upgrade menus are NOT closed by this result. Windows model enablement
remains gated until those integration responsibilities are covered.
