...
- [ ] Add binary validation tests using canonical WAV16k mono16-bit and:
...
- [ ] Source extraction tests require exactly one start/end anchor, preserve the
  original block verbatim and reject missing/duplicate anchors.
- [ ] Synthetic12x3x3x2 attempts exercise missing/duplicate tuples, changed hashes,
...
- [ ] Register push contracts, commit/push and inspect expected missing-module
  failure with `gh run view RUN -R xujxu/agents-chat --log-failed`.

## Task 2: Data validation and extractor

- [ ] Implement `extract_block(source)` using unique byte anchors from pinned
...
- [ ] Generate standalone source with original header includes and feature
...
- [ ] Generate CMake project: C++17, Release, MSVC static runtime, Windows UTF8
...
- [ ] Implement bounded `validate_wav`, `validate_pcm`, `validate_feature` and
...
- [ ] CI Python runner `run_native(binary, args, helper=None, timeout=120)`:
...
- [ ] On exit or cancellation, reap the child and ensure owned descendants are
...
- [ ] Add subprocess contracts: UTF8 success, empty/invalid/oversize stdout,
...
- [ ] `prepare` rebuilds frozen100 via existing script and compares the selected12
...
- [ ] Consumer validates BOTH entire producer artifacts before launching models,
...
- [ ] For repetitions1/2/3, sorted sample IDs, input sources `wav/linux/win32`,
...
- [ ] Aggregate checks exact216 tuples and matching producer/package/history
...
- [ ] Contracts on push; expensive jobs only on workflow_dispatch. Producers
...
- [ ] Commit/push, wait for red then green contracts, dispatch:

```bash
gh workflow run voice-feature-exchange.yml -R xujxu/agents-chat --ref experiment/voice-natural-long
```

- [ ] Inspect failed infrastructure remotely, fix and push if needed. Complete
