...
- [ ] Add a synthetic frozen100 fixture with60ASCEND/40AISHELL-4;50short,
...
- [ ] Reject missing/duplicate/unexpected tuples, source/ref/package changes,
...
- [ ] Register Python contracts on push; commit/push and inspect red:
...
- [ ] Implement `browser_report(samples, rows, original, baseline)` with exact
...
- [ ] Evidence CLI validates exact sample manifests, package role hashes,
...
- [ ] Push and require green Python contracts in Actions before real collection.

## Task3: Observational recorder helper and fixture coverage

- [ ] Implement `installBrowserCapture(page)` using `page.addInitScript`.
...
- [ ] Start source playback after the real UI enters recording. Keep100ms
...
- [ ] Dispatch original fetch first, then copy Blob bytes and clone response
...
- [ ] Fixture tests exercise success and exact payload, stop/HTTP/UI ordering,
...
- [ ] Add `voice-installed-browser.spec.ts` gated by
...
- [ ] Extend the existing runner with an explicitly validated optional
...
- [ ] Explicitly typecheck excluded test collectors and run focused fixture
...
- [ ] Reuse frozen runtime/model archive URLs and hashes from the spec; no
...
- [ ] Workflow: fast push contracts; manual expensive jobs only. Two platform
...
- [ ] Commit/push and dispatch after green contracts:
...
- [ ] Persist run/commit/artifact IDs, numerical gates, limits and open Win11/
